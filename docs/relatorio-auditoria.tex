...
\newcommand{\nc}{\textcolor{nc}{\textbf{N.C.}}}
\newcommand{\ne}{\textcolor{ne}{\textbf{N.E.}}}
\newcommand{\na}{\textbf{N/A}}
...
\midrule
3.1 & Comunicação protegida por TLS/HTTPS & \ne & Produção: via reverse proxy (Cloudflare Tunnel/Nginx). Desenvolvimento local: HTTP loopback (aceitável para ambiente controlado) \\
3.2 & Bloqueio de conexões não seguras & \ne & Implementado no nível do reverse proxy em produção; não enforced internamente \\
3.3 & Evidência de tráfego cifrado & \ne & Configuração de produção com Cloudflare documenta TLS 1.3. Sem evidência de certificado local \\
3.4 & Dados sensíveis cifrados em repouso & \nc & Senhas: cifradas (bcrypt). data.json com histórico de conversas: não cifrado em repouso \\
...
\begin{lstlisting}[language=JavaScript, caption={Validação de domínio no media proxy}]
...
\begin{lstlisting}[language=JavaScript, caption={Funções de cifragem segura para persistência}]
...
\begin{lstlisting}[language=JavaScript, caption={Rate limiter em memória sem dependências externas}]
...
\begin{lstlisting}[caption={package.json com versoes fixadas e hashes via package-lock.json}]
{
